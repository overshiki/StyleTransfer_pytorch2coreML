...
from simple_transfer import Vgg16
...
class StyleLoaderImage():
    def __init__(self, style_img, cuda=True):
        self.style_img = style_img
        img = Image.open(self.style_img).convert('RGB')
        self.style_size = img.size[0] * img.size[1]
        self.cuda = cuda
    ...
    def get(self):
        style = tensor_load_rgbimage(self.style_img, self.style_size)
        style = style.unsqueeze(0)
        style = preprocess_batch(style)
        if self.cuda:
            style = style.cuda()
